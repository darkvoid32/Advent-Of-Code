...
def find_common(puzzle):
    first = puzzle_split[0]
    second = puzzle_split[1]
    third = puzzle_split[2]
    return(''.join(set(first).intersection(second).intersection(third)))

while len(puzzle_split) > 2:
    common = find_common(puzzle_split)
    print(common)
    char_ord = ord(common)
    if char_ord >= 65 and char_ord <=90:
        total = total + char_ord - 65 + 27
    elif char_ord >= 97 and char_ord <= 122:
        total = total + char_ord - 97 + 1
